...
def fetch_stock_data(ticker):
    """Fetches the latest stock data for a given ticker symbol using yfinance."""
    stock = yf.Ticker(ticker)
    
    # Fetch the most recent history (last day to ensure current session data is included)
    history = stock.history(period="1d", interval="1m")
    
    if history.empty:
        raise Exception(f"No data found for ticker {ticker}. Possibly delisted or market closed.")
    
    # Extract the most recent row of data
    latest_row = history.iloc[-1]
    latest_time = latest_row.name.to_pydatetime()  # Convert index timestamp to datetime
...
# Got rid of alphavantage, fount python library with no key needed 1 min level access access (yfinance)
# Hoarding the old code in case this api is ever required.
